...
const PostCard = ({ post }: PostCardProps) => {
  // Derive initials dari avatar atau author
  const initials = (post.avatar ?? post.author).slice(0, 2).toUpperCase();

  // Format timestamp — pakai field timestamp (mock) atau created_at (API)
  const time =
    post.timestamp ??
    (post.created_at
